...
export default function PortfolioPage() {
  ...
  const { portfolio, common } = dictionary
  ...
  const initPortfolio = useCallback(async () => {
    ...
    if (existing) {
      ...
      const { data: created } = await supabase
        ...
        .select('id')
        .single()
      if (!created) { setLoading(false); return () => undefined }
    ...
    for (const row of allAnalyses ?? []) {
      ...
      if (ticker && !latestMap.has(ticker)) {
        latestMap.set(ticker, { cvar_95: row.cvar_95, verdict_action: row.verdict_action as VerdictAction | null })
      }
    }
  ...
  return (
    <TerminalShell>
      <div className="mb-8">
        <div className="font-mono text-[0.58rem] tracking-[0.18em] text-primary uppercase mb-2">{portfolio.eyebrow}</div>
        <h1 className="font-display text-xl font-bold text-obsidian-on">{portfolio.title}</h1>
        <p className="mt-2 text-obsidian-on-var text-[0.85rem]">
          {portfolio.descriptionLead}{' '}
          <a
      ...
      {loading ? (
        <div className="font-mono text-[0.75rem] text-obsidian-on-var">{portfolio.loading}</div>
      ) : assets.length === 0 ? (
        <EmptyState
          eyebrow={portfolio.empty.eyebrow}
          title={portfolio.empty.title}
          description={portfolio.empty.description}
          action={
            <a
              href="https://t.me/velveteen_stochasto_green_bot"
              target="_blank"
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-obsidian-outline-var">
                <th className="p-4 font-mono text-[0.58rem] text-obsidian-outline tracking-wider uppercase">{portfolio.table.ticker}</th>
                <th className="p-4 font-mono text-[0.58rem] text-obsidian-outline tracking-wider uppercase">{portfolio.table.cvar}</th>
                <th className="p-4 font-mono text-[0.58rem] text-obsidian-outline tracking-wider uppercase">{portfolio.table.verdict}</th>
                <th className="p-4 font-mono text-[0.58rem] text-obsidian-outline tracking-wider uppercase">{portfolio.table.added}</th>
                <th className="p-4 font-mono text-[0.58rem] text-obsidian-outline tracking-wider uppercase">{portfolio.table.manage}</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={asset.assetId} className="hover:bg-obsidian-mid transition-colors">
                    ...
                    <td className="p-4 font-mono text-[0.72rem] text-obsidian-outline">
                      {new Date(asset.added_at).toLocaleDateString(locale)}
                    </td>
                    <td className="p-4">
                      <button
                        type="button"
                        onClick={() => removeTicker(asset.assetId)}
                        title={portfolio.removeTitle}
